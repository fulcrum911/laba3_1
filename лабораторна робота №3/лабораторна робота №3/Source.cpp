...
void main()
{
	...
	InsertTree(&root, 53);
	InsertTree(&root, 49);
	cout << "Елементи бiнарного дерева" << endl;
	PrintTree(root);
	cout << endl;
	cout << "Максимальний елемент =" << getMax(root, -1000) << endl;
	cout << "Мiнiмальний  елемент =" << getMin(root, 1000) << endl;
	cout << "Сума заданих елементiв = " << getSum(root, getMax(root, -1000), getMin(root, 1000)) << endl;
	Delete(&root, 97);
	Delete(&root, 8);
	Delete(&root, 9);
	Delete(&root, 53);
